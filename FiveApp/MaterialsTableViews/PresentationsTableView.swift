...
protocol PresentationsDelegate: AnyObject {
    func addPresetationButtonWasTapped()
}

struct PresentationsCellModel {
    let title: String
    let data: Data
}

class PresentationsTableView: UITableView {
    
    var navigationController: UINavigationController!
    
    var presentationModels = [PresentationsCellModel]()
    
    weak var presentationDelegate: PresentationsDelegate?
    ...
    func configure() {
        delegate = self
        dataSource = self
        register(PresentationsCell.self, forCellReuseIdentifier: "cellId")
    }
    ...
    func createTableHeader() -> UIView? {
        ...
        labelTitle.text = "Prezentacije"
        
        
        plusImageView.isUserInteractionEnabled = true
        self.isUserInteractionEnabled = true
        let gesture = UITapGestureRecognizer(target: self, action: #selector(didTapAddPresentationButton))
    ...
    @objc private func didTapAddPresentationButton() {
        presentationDelegate?.addPresetationButtonWasTapped()
    }
    ...
    func reload(downloadURL: String) {
        ...
        URLSession.shared.dataTask(with: url, completionHandler: { [weak self] data, _ , error in
            ...
            strongSelf.presentationModels.append(PresentationsCellModel(title: url.lastPathComponent, data: data))
    ...
    func fetchPresentationsData() {
        let path = "presentations/"
...
extension PresentationsTableView: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cellId", for: indexPath) as! PresentationsCell
        cell.setup(model: presentationModels[indexPath.row])
        return cell
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        pdfVC.setData(data: presentationModels[indexPath.row].data)
        navigationController.pushViewController(pdfVC, animated: true)
    }
    
}

extension PresentationsTableView: UIDocumentPickerDelegate {
    func documentPicker(_ controller: UIDocumentPickerViewController, didPickDocumentAt url: URL) {
        ...
        do {
            ...
            let fileName = "presentations/\(url.lastPathComponent)"
